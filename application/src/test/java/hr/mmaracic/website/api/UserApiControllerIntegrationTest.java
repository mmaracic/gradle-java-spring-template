...
class UserApiControllerIntegrationTest {
    ...
    @Test
    void testController() {
        assertThat(applicationContext.containsBean("userApiController"), equalTo(true));
    }
    ...
    void testGetUserByName() throws Exception {
        MvcResult result = mvc.perform(get("/user/by-name").param("name", "user"))
                .andDo(log()).andExpect(status().isOk())
                .andReturn();
    ...
    void testGetUnknownUserByName() throws Exception {
        MvcResult result = mvc.perform(get("/user/by-name").param("name", "unknown"))
